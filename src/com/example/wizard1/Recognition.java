...
public abstract class Recognition {
     ...
     * @return ���������� �������� ������ ��� "-", ���� ������ �� ����������.
     */
    public static Shape recognize(ArrayList<Vector2d> projection, Handler mHandler) {
        PC = projection;
        calcAngles();

        if (isCircle()) {
            mHandler.obtainMessage(WizardFight.AppMessage.MESSAGE_SET_PROJECTION.ordinal(), 0, 0, PC)
                    .sendToTarget();
            return Shape.CIRCLE;
        }
        ArrayList<ArrayList<Vector2d>> lines = isEnd(30, 5);
        mHandler.obtainMessage(WizardFight.AppMessage.MESSAGE_CLEAR_PROJECTION.ordinal(), 0, 0,0)
                .sendToTarget();
        for (ArrayList<Vector2d> line : lines)
            mHandler.obtainMessage(WizardFight.AppMessage.MESSAGE_ADD_PROJECTION.ordinal(), 0, 0, line)
                    .sendToTarget();
        switch (lines.size()) {
            case 3:
